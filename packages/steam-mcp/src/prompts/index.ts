...
export function registerSteamPrompts(server: McpServer, context: SteamMcpContext): void {
  ...
  registerPromptShallow(
    ...
    'steam_discovery_router',
    {
      ...
      description: 'Guide an agent through evaluating plausible Steam discovery surfaces, then choosing one primary discovery path and at most one adjacent fallback across the current validated Steam discovery surface.',
    ...
    (rawArgs: unknown) => {
      ...
      return {
        description: 'Workflow for routing broad Steam discovery requests by evaluating plausible surfaces first, then executing one primary path plus at most one adjacent fallback without widening the MCP surface.',
        messages: [{
          role: 'user',
          content: {
            type: 'text',
            text: [
              'Use the Steam MCP to evaluate plausible discovery surfaces for a broad request, then route safely across the current validated discovery surface.',
              ...
              'Workflow:',
              '1. Call steam_status first and confirm the selected Steam user, whether `STEAM_API_KEY` is available, and which discovery surfaces are plausible candidates: library-centric, wishlist-centric, store-centric, release-centric, featured/editorial, similar-title, or recent-play driven.',
              '2. Evaluate all plausible candidate discovery surfaces against the request and prerequisites, then choose exactly one primary path before calling any discovery tool or prompt.',
        ...
        messages: [{
          ...
          content: {
            ...
            text: [
              ...
              countryCode ? `Requested country code: ${countryCode}.` : 'Requested country code: leave unset unless locale scoping is needed.',
              ...
              '4. Use language and countryCode only when the user wants locale-scoped official catalog results; otherwise leave them unset.',
