...
description: "Use when the user wants safe read-only Steam library analysis, deterministic recommendations, store enrichment, exports, or steam:// link generation. Trigger phrases: 'organize my library', 'find similar games', 'recommend from my Steam library', 'export my Steam library', 'generate Steam links'."
---

# Steam Library Curator

Use this skill for read-heavy Steam library workflows built on top of the Steam MCP prompt `steam_library_curator`.

## When to Use

- The user wants to analyze or organize their Steam library without mutating Steam-owned state.
...
- The user wants store enrichment, JSON/Markdown export, or `steam://` launch/store/library/community links.
...
4. Add `steam_find_similar` or `steam_store_search` only when you need deterministic comparison or enrichment.
...
If the user wants to reorganize categories, favorites, or hidden flags, stop this workflow and switch to the `steam-collection-planner` skill.
