...
- The user wants help deciding between library-first, release-first, featured/editorial, authenticated catalog, public-store lookup, or similarity-driven discovery.
...
6. Keep fallbacks adjacent and explicit: release or featured discovery may fall back to `steam_store_query`; `steam_store_query` may fall back to `steam_store_search`; recently played or library analysis may use `steam_find_similar` only as a follow-up comparison step.
...
steam_library_curator | steam_deck_backlog_triage | steam_recently_played | steam_find_similar | steam_release_scout | steam_featured_scout | steam_store_query | steam_store_search
