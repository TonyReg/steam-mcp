...
- `steam_discovery_router` — broad-intent discovery guidance that evaluates plausible discovery surfaces, then chooses one primary path and at most one adjacent fallback across the current validated Steam MCP discovery surface
...
- `steam_discovery_router` is a prompt-only guidance surface over the existing validated tools; it evaluates plausible discovery surfaces, then chooses one primary path plus at most one adjacent fallback and does not add a new discovery tool or Steam endpoint contract
